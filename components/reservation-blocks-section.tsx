...
function transformRoomTypesToABS(
  roomTypes: RoomType[],
  ...
  return roomTypes.map((room) => {
    ...
    const roomDescription = getMultilingualValue(
      room.description,
      'Experience supreme luxury with divine views and premium amenities',
    );
    const roomPrice = Number(room.base_price) || 0;
    const roomAmenities = Array.isArray(room.amenities) ? room.amenities : [];
    const roomImages =
      room.images && room.images.length > 0 ? room.images : [room.main_image];
...
function transformCustomizationOptionsToABS(
  ...
  options.forEach((option) => {
    ...
    const optionDescription = getMultilingualValue(
      option.description || {},
      `Premium ${optionName.toLowerCase()} with enhanced luxury amenities`,
    );
    ...
    if (!groupedOptions[category]) {
      groupedOptions[category] = {
        icon: category.toLowerCase().includes('bed')
          ? 'bed'
          : category.toLowerCase().includes('view')
            ? 'city-view'
            : category.toLowerCase().includes('feature')
              ? 'sparkles'
...
function transformSpecialOffersToABS(
  offers: SpecialOffer[],
  currentLanguage: string = 'en',
) {
  // Helper function to extract multilingual string value
...
function mapPriceTypeToABS(
  priceType: string,
): 'perStay' | 'perPerson' | 'perNight' {
  switch (priceType) {
    case 'per_person':
      return 'perPerson';
    case 'per_night':
...
export default function ReservationBlocksSection({
  ...
  const { roomTypes, customizationOptions, specialOffers, loading, error } =
    useReservationContent(currentLanguage);
  ...
  const { sections: customizationSections, sectionOptions } = useMemo(() => {
    const result = transformCustomizationOptionsToABS(
  ...
  const transformedOffers = useMemo(() => {
    const transformed = transformSpecialOffersToABS(
      safeSpecialOffers,
      currentLanguage,
    );
    ...
    <div className="grid grid-cols-3 gap-6 pb-12">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex-1 h-[max(650px,calc(100vh-350px))]">
        <div className="border-b border-gray-100 bg-gray-50/50 px-4 py-3">
          <h2 className="text-3xl font-semibold text-gray-900 mb-1">
            {getABSTranslation('chooseYourSuperiorRoomLabel') ||
              'Room Upgrades'}
          </h2>
          <p className="text-xs text-gray-600">
            {getABSTranslation('selectRoomDescription') ||
              'Upgrade to supreme luxury with divine ocean views and enhanced amenities'}
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex-1 h-[max(650px,calc(100vh-350px))]">
        <div className="border-b border-gray-100 bg-gray-50/50 px-4 py-3">
          ...
          <p className="text-xs text-gray-600">
            {getABSTranslation('offersDescription') ||
              'Experience supreme luxury with divine views and premium amenities'}
          </p>
        </div>
        {transformedOffers.length > 0 &&
        Object.keys(initialOfferSelections).length > 0 &&
        transformedOffers.every(
          (offer) => initialOfferSelections[offer.id] !== undefined,
        ) ? (
          <ABS_SpecialOffers
            ...
            labels={{
              ...
              removeOfferLabel:
                getABSTranslation('removeOffer') || 'Remove from List',
              decreaseQuantityLabel:
                getABSTranslation('decreaseQuantity') || 'Decrease quantity',
              increaseQuantityLabel:
                getABSTranslation('increaseQuantity') || 'Increase quantity',
              selectDateLabel: getABSTranslation('selectDate') || 'Select Date',
              selectDateTooltip:
                getABSTranslation('selectDateTooltip') ||
                'Choose the date for this service',
              dateRequiredLabel:
                getABSTranslation('dateRequired') || 'Date selection required',
              selectDatesLabel:
                getABSTranslation('selectDates') || 'Select Dates',
              selectDatesTooltip:
                getABSTranslation('selectDatesTooltip') ||
                'Choose dates for this service',
              availableDatesLabel:
                getABSTranslation('availableDates') || 'Available Dates',
              noAvailableDatesLabel:
                getABSTranslation('noAvailableDates') || 'No available dates',
              ...
              multipleDatesRequiredLabel:
                getABSTranslation('multipleDatesRequired') ||
          ...
          <div className="flex items-center justify-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <div className="text-center">
              ...
              <p className="text-gray-500 font-medium text-sm">
                {getABSTranslation('loadingOffers') ||
