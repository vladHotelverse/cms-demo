...
import {
  ...
  useSelectionErrorHandler,
} from '@/components/selection-error-boundary';
...
const ReservationDetailsTab = memo(function ReservationDetailsTab({
  ...
  const { addRoom, addExtra, selectedRooms, selectedExtras } =
  ...
  const buttonStates = useMemo(() => {
    ...
    selectionSummary.hasItems,
    isClearingSelections,
    ...
    async (item: any) => {
      try {
        ...
        if (result.success) {
          onShowAlert(
            'success',
            `${item.name || 'Service'} added to selection`,
          );
        } else {
          onShowAlert(
            'error',
            result.errors?.[0]?.message || 'Failed to add service',
          );
        }
      } catch (error) {
      ...
      try {
        ...
        if (isCarouselData) {
          ...
          transformedRoom = {
            ...
            roomType:
              room.title || room.name || room.roomType || 'Superior Room',
        ...
        if (result.success) {
          ...
          onShowAlert(
            'error',
            result.errors?.[0]?.message || 'Failed to add room',
          );
        }
      } catch (error) {
  ...
  const handleSelectAttribute = useCallback(
    async (attribute: any, itemKey: string) => {
      try {
        const {
          updateRoomCustomizations,
          addRoomFromCustomization,
          selectedRooms,
        } = useUserSelectionsStore.getState();
        ...
        if (selectedRooms.length > 0) {
          ...
          if (result.success) {
            onShowAlert(
              'success',
              `${attribute.name} added to room attributes`,
            );
          } else {
            onShowAlert(
              'error',
              result.errors?.[0]?.message || 'Failed to add attribute to room',
            );
          }
        } else {
          // No room exists, create a new room with this customization
          const result = await addRoomFromCustomization(
            customization,
            reservationInfo,
          );
          ...
          if (result.success) {
            onShowAlert(
              'success',
              `Room created with ${attribute.name} attribute`,
            );
          } else {
            onShowAlert(
              'error',
              result.errors?.[0]?.message ||
                'Failed to create room with attribute',
            );
          }
        }
      } catch (error) {
      ...
      try {
        ...
        if (!optionId) {
          ...
          if (selectedRooms.length > 0) {
            ...
            await updateRoomCustomizations(
              existingRoom.id,
              storeCustomization,
              0,
            );
        ...
        if (selectedRooms.length > 0) {
          ...
          if (result.success) {
            onShowAlert(
              'success',
              `${optionLabel} added to room customizations`,
            );
          } else {
            onShowAlert(
              'error',
              result.errors?.[0]?.message ||
                'Failed to update room customization',
            );
          }
        } else {
          // No room exists, create a new room with this customization
          ...
          if (result.success) {
            onShowAlert(
              'success',
              `Room created with ${optionLabel} customization`,
            );
          } else {
            onShowAlert(
              'error',
              result.errors?.[0]?.message ||
                'Failed to create room with customization',
            );
          }
        }
      } catch (error) {
  ...
  const handleSelectionSummaryCustomization = useCallback(
    (roomId: string, customizations: any, total: number) => {
  ...
  const handleOfferBook = useCallback(
    async (offerData: any) => {
      ...
      try {
        ...
        if (result.success) {
          onShowAlert(
            'success',
            `${offerData.title || 'Offer'} added to selection`,
          );
        } else {
          onShowAlert(
            'error',
            result.errors?.[0]?.message || 'Failed to add offer',
          );
        }
      } catch {
  ...
  const handleClearAllSelections = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        userSelections.bidding.length === 0
      ) {
        onShowAlert(
          'info',
          t('currentLanguage') === 'es'
  ...
  const handleConfirmSelections = useCallback(() => {
    ...
    const totalItems =
      itemCounts.rooms + itemCounts.extras + itemCounts.bidding;
    ...
    const pendingItems = [
      ...
      ...userSelections.extras.filter(
        (item) => item.status === 'pending_hotel',
      ),
      ...userSelections.bidding.filter(
        (item) => item.status === 'pending_hotel',
      ),
    ];
          ...
          {viewMode === 'list' ? (
            ...
            <div className="w-full">
              ...
              <ABS_RoomSelection
                title={
                  t('currentLanguage') === 'es'
                    ? '¡Elige tu Número de Habitación!'
                    : 'Choose your Room Number!'
                }
                description={
                  t('currentLanguage') === 'es'
                    ? 'Selecciona la habitación exacta en la que quieres alojarte'
                    : 'Select the exact room you want to stay in'
                }
